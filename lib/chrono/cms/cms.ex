defmodule Chrono.CMS do
  ...
  def get(type, filter_fn, return_fn, default_value, caller) do
    with {:ok, rs} <-
           Chrono.CMS.get(type)
           |> (&filter_fn.(&1)).()
           |> Chrono.Either.either() do
      return_fn.(rs)
    else
      {:error, err} ->
        Logger.warn(
          "#{inspect(__MODULE__)}-#{inspect(caller)}:#{inspect(err)} Danger Will Robinson there is no image falling back local"
        )
  ...
  def get(type) do
    with {:ok, result} when result != nil <- get!(type) |> Chrono.Either.either() do
      result
    else
      {:ok, nil} -> []
      {:error, e} -> e
  ...
  defp to_content(_content_type, entry),
    do: to_content(entry) |> Map.put(:body, entry |> get_in(["fields", "description"]))

  defp get_linked_resource(nil), do: nil

  defp get_linked_resource(xs),
    do: xs |> Enum.map(&to_content(&1 |> get_in(["sys", "contentType", "sys", "id"]), &1))
end
